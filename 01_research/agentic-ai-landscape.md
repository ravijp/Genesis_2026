...
*(Sources for this section are numbered **E1–E62** below.)*

> **⚠️ Critical source-quality warning (act on this in the competition).** A whole tier of SEO "leaderboard aggregator" sites (BenchLM.ai, Steel.dev leaderboards, pricepertoken.com, coasty.ai, morphllm.com) **systematically fabricate model names and scores** — e.g., "GLM-5.2 at 99.1% tau2," "Claude Mythos 5 at 95.5% SWE-bench," "GPT-5.6 Sol at 92.2% BrowseComp." **None of these are real released models/scores.** Some figures elsewhere in this brief predate this correction; **the numbers in this section supersede them** and are grounded in primary sources (official leaderboards, arXiv, Princeton HAL, METR, Scale, Sierra). For Genesis: cite primary benchmarks with the exact split and self-reported-vs-verified status — vague "we scored X%" claims are exactly what a rigorous AI judge discounts.
