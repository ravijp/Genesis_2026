...
- `test_read_live.py` — 22 tests on the turn-by-turn read. The two that matter: a `RecordingExtractor` pins that prefixes genuinely grow, never repeat and never reorder (a model shown the whole transcript at every step would make the animation a lie), and the final step's messages are asserted equal to the batch read's, which is what makes narration and the ledger the same belief `[stable]`
