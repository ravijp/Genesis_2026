...
- `tenants.py` — **the multi-enterprise layer, and it is a configuration layer only.** A tenant is one `RunConfig` (its own corpus and its own decay half-lives), one fixed alert threshold, and one map from the four canonical `OwningTeam` slots onto the names that enterprise uses. There is no per-tenant scorer, prompt or code path. The team map is **display-only on purpose**: widening `OwningTeam` per tenant would put an enterprise-supplied string inside the model's decision contract, and the closed `Literal` plus `extra="forbid"` is what stops a model inventing a routing destination. `public()` withholds the seed and every corpus parameter, so nothing tenant-shaped carries the one value those regenerate from. The run **manifest** still carries it, deliberately — provenance a judge cannot reproduce is worth little — so the guarantee is the narrower, mechanical one: no answer-key field is present and no code path in the UI regenerates anything `[stable]`
...
- `test_stream.py` — 14 tests on the arrival stream as **properties, not pinned outputs**: frames agree with `memory.py`, the order is genuinely interleaved across customers (grouped-by-customer would still animate and would still be a batch), `score_before` is taken on the same day so a delta cannot fold in decay, `ledger_size` is monotonic (never-discard, asserted mechanically), a customer opens at most one case, and the payload guard fires on a nested answer-key field `[stable]`
- `test_tenants.py` — 12 tests that the three deployments genuinely differ: distinct seeds and config hashes (a shared seed is the same customers under three logos), half-lives and thresholds that actually diverge, a team map covering every slot in `OwningTeam` read off the contract rather than retyped, and a `public()` payload that withholds the seed `[stable]`
- `test_stream_server.py` — 8 tests on the demo server's three safety properties: `do_GET` is the only handler (the routes are unauthenticated on purpose, which is safe only while they are all reads), no outbound contact surface, loopback binding read off the AST, and four path-traversal forms driven through the real `_static` rather than inferred from the source `[stable]`
