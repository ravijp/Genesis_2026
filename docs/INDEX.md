...
- `test_provision.py` — 15 tests on the queue coupling that broke the deployed path. The load-bearing one asserts a **property across files**: every queue-backed function in `deploy.FUNCTIONS` must have a visibility timeout at least its own timeout, because AWS refuses the event source mapping otherwise and both mappings failed that way on 2026-09-02. The rest pin the reconcile path — an existing queue at the SQS default is repaired rather than skipped, a correct one is not rewritten at all, and neither an int on our side nor a re-serialised `RedrivePolicy` may look like drift `[stable]`
- `test_feed.py` — 21 tests on the feeder's two ways of lying. No answer-key field may reach the wire (checked against the **real** corpus, since a hand-built conversation proves nothing about the generator's), the payload round-trips through the deployed `parse_conversation` with every turn index intact, and each customer's conversations are grouped in day order. The rest make the verifier falsifiable: one parametrised case per `verify()` branch, asserted in the *failing* direction, because a verifier with an unreachable failure branch cannot be told apart from one that always passes `[stable]`
...
- `provision.py` — idempotent boto3 provisioning, **dry-run by default**. Three tables, three queues with DLQ redrive. Schema imported from `stores.TABLE_SPECS` so the two cannot drift. Teardown has no code path that can delete the ledger. Queues are **created and reconciled**, not merely created: an existing queue's mutable attributes are compared against live and corrected, because both queues already existed at the SQS default visibility timeout and a create-only tool would have reported SKIP while the defect survived. `VisibilityTimeout` is derived as 6x the consuming function's timeout read out of `deploy.FUNCTIONS`, so the two files cannot drift again. Known permission gaps print **PARKED** rows naming the gap, the owner and the workaround. PITR is enabled with a bounded retry: a table is ACTIVE seconds before its backups subsystem is, and the gap reports as `ContinuousBackupsUnavailableException`, which reads like a permission problem `[stable]`
- `feed.py` — **the end-to-end proof**: publishes one tenant's book to the deployed FIFO queue and then asserts the deployed result against the local pipeline. Predicts first — `memory.py` is the only scorer, and `aws/ingest.py` delegates to it, so ledger writes, crossings and every crossing score are knowable before a message is sent — and **exits non-zero when the deployment disagrees**, because a feeder that reports green either way is the `timeout … | tail` mistake again. One batch per customer in day order (`MessageGroupId` is the customer id) because accumulation is order-dependent; `MessageDeduplicationId` is the conversation id, so a re-feed is a no-op in SQS and again on the ledger. Holds the corpus, so it lives in `tools/` and `wire_payload()` is the boundary: the five fields `parse_conversation` requires, built from named attributes, key set asserted `[stable]`
...
- `impact/onepager-accuracy-cost-latency.md` — **required deliverable ②**: the internal evidence page. Leads with the desk table (which review desks exist under which reader), then reader cost/latency/fidelity, investigation, verdicts and routing, then the sweep. Every figure with its denominator and the command that reproduces it keylessly; a "what is not measured" section that names the open gaps with prices on them `[stable]`
...
- `cache/investigator-bedrock.jsonl` — AT-57/AT-58's cache: 50 verdict-loop investigations keyed Haiku 4.5 on Bedrock (2026-08-31: **29 / 50** verdicts, **27 / 48** routing), replayed by `tools/verdict_accuracy.py` and scored free by `tools/routing_accuracy.py`. AT-57 replays **only** at `--per-arm 25 --customers 2400`; a different sample size draws different cases and returns `provider_error` `[generated]`
- `cache/extractor.jsonl` — the model reader's cache behind the CFPB benchmark (0.8214 strict recall), the streamed-demo reads and the **reader-coverage model arm** (2026-08-31: complaints 0 / 20 → 20 / 20, vulnerability 0 / 20 → 19 / 20, at $1.58 per 1,000): one file per model by `extractor_cache_path()`, this is the default (Haiku 4.5) path `[generated]`
- `cache/extractor-coverage.jsonl` — **superseded, kept as the record of a retracted measurement.** The pre-Phase-C coverage run: both readers over 288 sampled conversations, behind the "model reader revives retention (0 → 9 / 20), loses collections (4 → 1)" figures that are **no longer published anywhere**. The current coverage run reads from `cache/extractor.jsonl`, the default per-model path `[generated]`
