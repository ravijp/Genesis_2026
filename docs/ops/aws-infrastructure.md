...
| **SQS** | `earshot-dev-transcripts.fifo` (+ `-dlq.fifo`) · `earshot-dev-investigations` (+ `-dlq`) | **Four queues.** Both consumed queues redrive at 3 receives; a FIFO source needs a FIFO DLQ, hence two. Visibility timeouts are **6× the consuming function's** (360s / 1800s), derived from `deploy.FUNCTIONS` — below the function timeout AWS refuses the event source mapping outright. The transcripts DLQ was added 2026-09-03: until then that queue had none, so a poison transcript retried for the full 4-day retention and, being FIFO, blocked that customer's whole stream |
| **Lambda** | `earshot-dev-ingest` · `earshot-dev-investigate` · `earshot-dev-api` | Redeployed 2026-09-03, python3.13, zip sha `17c99889f6b1`, all passing `zenon-poc-lambda-execution`. Both handlers run the **offline** provider (keyless, no spend); `deploy.py --provider bedrock` flips it. **No longer inert:** both event source mappings are Enabled with `ReportBatchItemFailures`, and the whole Northwind book has been fed through end to end (`tools/feed.py`) |
| **CloudWatch alarms** | 6, `earshot-dev-*` | Created 2026-08-28. **No actions on any of them** — no SNS on this account, so they go red in a console and page nobody. **Four of the six were guarding nothing until 2026-09-03:** EMF omitted the `_aws.Timestamp` it requires, so CloudWatch dropped every `Earshot` metric while storing the log lines, and `TreatMissingData=notBreaching` rendered them **OK** rather than INSUFFICIENT_DATA. The two on `AWS/SQS` were always real. No alarm has yet transitioned to ALARM, so the thresholds are reasoned, not observed. `tools/alarms.py` |
| **Reviewer API URL** | `https://omqdmdwrekgdrkybgerq7ggkji0soujn.lambda-url.us-east-1.on.aws/` | Function URL, `AuthType=AWS_IAM`. All six routes answer: `/health`, `/cases`, `/cases/{id}`, `/cases/{id}/reviews`, `/customers/{id}/ledger`, `/customers/{id}/conversations/{cid}`, plus 404 on an unknown path. Verified 2026-09-03 by `lambda invoke` against real DynamoDB, **not** over HTTP — signing the IAM Function URL is still open (a static page cannot) |
| **SSO start URL** | `https://d-906744472d.awsapps.com/start` | In `~/.aws/config` as sso-session `zenon`; profile `genesis` assumes `agentic-trio` in `859430413223`. **`AWS_PROFILE=genesis` must be set** — without it boto3 finds no credentials and the failure reads like a dead token |
