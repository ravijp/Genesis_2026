...
def test_arm_sample_precision_matches_evals_definition(swept) -> None:
    ...
    # `seed=` too: `random-rank` draws from it, so omitting it builds a DIFFERENT arm than the
    # fixture did and the comparison silently comes apart on that one arm only.
    arms = run_all_arms(signals, run.scoring, seed=SEEDS[0])
